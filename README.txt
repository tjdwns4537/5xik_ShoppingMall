...
ㅇ 도메인 모델 분석
 ...
 - 회원-주문의 관계 
  : 회원은 여러번 주문할 수 있다. 반대로 주문의 경우 반드시 한명의 회원에 속해야함

 - 회원-상품의 관계
  : 물품을 구매했을 때 영수증을 생각해보면 이해가 쉽다. 하나의 주문에는 여러 상품을 가질 수 있음
    그리고 하나의 상품은 여러 주문에 속할 수 있다. 따라서 주문과 상품은 다대다 관계이다.
    테이블 설계에 있어서 다대다 관계는 문제가 있으므로, [ 주문, 상품 ] 의 다대다 관계를 풀어주기 위해
    주문상품이라는 테이블을 만들어 1:다,다:1 관계로 다대다 관계를 풀어줍니다.
    즉, OrderItem을 영수증이라 생각하면 됩니다.
